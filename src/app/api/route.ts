...
export async function POST(req: NextRequest) {
  ...
  // Free users (no pass): enforce the 1 minute/day limit SERVER-SIDE.
  // A tampered client that skips usage reporting can no longer match
  // forever — once 60s are on record, matching is refused until tomorrow
  // (or until a pass is purchased).
  if (effectiveTier === 0) {
    ...
    if (used >= 60) {
      return NextResponse.json(
        { error: 'Daily free time is over — get a pass for unlimited calls' },
        { status: 403 }
      );
    }
  }
